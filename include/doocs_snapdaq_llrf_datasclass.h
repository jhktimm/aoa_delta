...
//     int sizeof_this() {
//         int size = 0;
//         size += doocschannel.size();
//         size += section.size();
//         size += station.size();
//         size += type.size();
//         size += sizeof(std::vector<float>) + (sizeof(float) * forward_signal.size());
//         size += sizeof(std::vector<float>) + (sizeof(float) * forward_phase.size());
//         size += sizeof(std::vector<float>) + (sizeof(float) * reflected_signal.size());
//         size += sizeof(std::vector<float>) + (sizeof(float) * reflected_phase.size());
//         size += sizeof(std::vector<float>) + (sizeof(float) * probe_signal.size());
//         size += sizeof(std::vector<float>) + (sizeof(float) * probe_phase.size());
//         return size;
//     }
};
...
//         size += sizeof(time);
//         size += str.size();
//         size += sizeof(std::vector<doocs_snapdaq_data_channel>);
//         for ( auto s : channel ) size += s.sizeof_this();
//         return size;
//     }
...
// bool operator==(const macro_pulse & lhs, const macro_pulse & rhs) {
...
class doocs_snapdaq_llrf_dataclass
{
public:
	std::vector<macro_pulse> mps;
	
	doocs_snapdaq_llrf_dataclass() {}
	doocs_snapdaq_llrf_dataclass(std::string filename) {this->read(filename);}
	~doocs_snapdaq_llrf_dataclass() {}

    friend std::ostream & operator << (std::ostream & out, const doocs_snapdaq_llrf_dataclass& obj) {
        auto s = obj.mps.size();
        out.write((char*) &s, sizeof(s));
        for ( auto o : obj.mps) out << o;
        return out;
    }    
    friend std::istream & operator >> (std::istream & in, doocs_snapdaq_llrf_dataclass & obj) {
        auto s = obj.mps.size();
        in.read((char*) &s, sizeof(s));
        obj.mps.resize(s);
...
//     bool operator==(const doocs_snapdaq_llrf_dataclass & obj) {
//         return (mps == obj.mps) ;
//     }
};
