                   ...
                   const emxArray_creal_T *y_mC, double fs, double f0)
{
  ...
  i = r6->size[0];
  r6->size[0] = absb;
  emxEnsureCapacity((emxArray__common *)r6, i, sizeof(int));
  absb = 0;
  for (i = 0; i <= idx; i++) {
    if (idx_u_mCal_off->data[i] > Rise->data[0] + (double)rampup_smpls) {
      r6->data[absb] = i + 1;
  ...
  emxInit_real_T(&r7, 2);
  if (u_mCal->size[0] < (int)idx_u_mCal_off->data[r6->data[0] - 1]) {
    i = r7->size[0] * r7->size[1];
    r7->size[0] = 1;
    r7->size[1] = 0;
    emxEnsureCapacity((emxArray__common *)r7, i, sizeof(double));
  } else if (idx_u_mCal_off->data[r6->data[0] - 1] == idx_u_mCal_off->data
             [r6->data[0] - 1]) {
    ndbl = idx_u_mCal_off->data[r6->data[0] - 1];
    i = u_mCal->size[0];
    absb = r7->size[0] * r7->size[1];
    r7->size[0] = 1;
    r7->size[1] = (int)floor((double)i - ndbl) + 1;
    emxEnsureCapacity((emxArray__common *)r7, absb, sizeof(double));
    absb = (int)floor((double)i - ndbl);
    for (i = 0; i <= absb; i++) {
      r7->data[r7->size[0] * i] = ndbl + (double)i;
    }
  } else {
    ndbl = floor(((double)u_mCal->size[0] - idx_u_mCal_off->data[r6->data[0] - 1])
                 + 0.5);
    apnd = idx_u_mCal_off->data[r6->data[0] - 1] + ndbl;
    cdiff = apnd - (double)u_mCal->size[0];
    absa = (unsigned int)fabs(idx_u_mCal_off->data[r6->data[0] - 1]);
    absb = u_mCal->size[0];
    ...
    if (fabs(cdiff) < 4.4408920985006262E-16 * (double)u1) {
      ...
      apnd = idx_u_mCal_off->data[r6->data[0] - 1] + (ndbl - 1.0);
    } else {
      ndbl++;
    }
    ...
    emxEnsureCapacity((emxArray__common *)r7, i, sizeof(double));
    if (idx > 0) {
      r7->data[0] = idx_u_mCal_off->data[r6->data[0] - 1];
      if (idx > 1) {
        r7->data[idx - 1] = apnd;
        absb = (idx - 1) / 2;
        for (i = 1; i < absb; i++) {
          r7->data[i] = idx_u_mCal_off->data[r6->data[0] - 1] + (double)i;
          r7->data[(idx - i) - 1] = apnd - (double)i;
        }

        if (absb << 1 == idx - 1) {
          r7->data[absb] = (idx_u_mCal_off->data[r6->data[0] - 1] + apnd) / 2.0;
        } else {
          r7->data[absb] = idx_u_mCal_off->data[r6->data[0] - 1] + (double)absb;
          r7->data[absb + 1] = apnd - (double)absb;
        }
      }
    }
  }
  ...
  apnd = idx_u_mCal_off->data[r6->data[0] - 1];
  ndbl = (double)r7->size[1] - 1.0;
  i = b_y_mC->size[0];
