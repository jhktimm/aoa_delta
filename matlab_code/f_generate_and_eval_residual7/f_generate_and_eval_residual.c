  ...
  if (mean(b_FA) > 3.0) {
    ...
    do {
      ...
      if (sum(b_PP) > 0.0) {
        ...
        i0 = dw_nom->size[0];
        dw_nom->size[0] = dw->size[0];
        emxEnsureCapacity_real_T1(dw_nom, i0);
        nm1d2 = dw->size[0];
        for (i0 = 0; i0 < nm1d2; i0++) {
          dw_nom->data[i0] = PP->data[(int)((70.0 + dw->data[i0]) - 2.0) - 1];
        }

        nm1d2 = dw_nom->size[0];
        for (i0 = 0; i0 < nm1d2; i0++) {
          PP->data[ii->data[i0] - 1] = dw_nom->data[i0];
        }
      } else {
        exitg1 = 1;
      }
    ...
    c_f_generate_online_UKF_residua(PA, PP, FA, FP, RA, RP, FS, cal_coeff, tau_m,
      K_m, X0, QL_nom, MeasNoiseVar, ProcessVar, res_full_mean,
      res_full_variance);
    ...
    for (b_ii = 0; b_ii < n; b_ii++) {
      for (i0 = 0; i0 < 2; i0++) {
        y->data[i0 + y->size[0] * b_ii] = res_full_mean->data[i0 +
          res_full_mean->size[0] * b_ii] - r_mean_nom[i0];
      }
    }
    ...
    for (i0 = 0; i0 < nm1d2; i0++) {
      c_FA->data[i0].re = y_mC->data[(int)Flattop->data[Flattop->size[0] * i0] -
        1].re - (u_mCal->data[(int)Flattop->data[Flattop->size[0] * i0] - 1].re
                 + r_mCal->data[(int)Flattop->data[Flattop->size[0] * i0] - 1].
                 re);
      c_FA->data[i0].im = y_mC->data[(int)Flattop->data[Flattop->size[0] * i0] -
        1].im - (u_mCal->data[(int)Flattop->data[Flattop->size[0] * i0] - 1].im
                 + r_mCal->data[(int)Flattop->data[Flattop->size[0] * i0] - 1].
                 im);
    }
    ...
    f_compute_detuning(y_mC, u_mCal, fs, QL_nom, dw_nom);
    i0 = b_FA->size[0];
    ...
    nm1d2 = Flattop->size[1];
    for (i0 = 0; i0 < nm1d2; i0++) {
      b_FA->data[i0] = dw_nom->data[(int)Flattop->data[Flattop->size[0] * i0] -
        1];
    }
    ...
    b_abs(c_FA, dw_nom);
    i0 = c_x->size[0];
    c_x->size[0] = dw_nom->size[0];
    emxEnsureCapacity_boolean_T(c_x, i0);
    nm1d2 = dw_nom->size[0];
    emxFree_creal_T(&c_FA);
    for (i0 = 0; i0 < nm1d2; i0++) {
      c_x->data[i0] = (dw_nom->data[i0] < 1.5);
    }

    emxFree_real_T(&dw_nom);
    nm1d2 = c_x->size[0];
    idx = 0;
    i0 = ii->size[0];
    ii->size[0] = c_x->size[0];
