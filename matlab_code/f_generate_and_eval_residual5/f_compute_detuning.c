  ...
  *u_mC, double fs, double QL, emxArray_real_T *dw)
{
  ...
  r1->size[0] = (int)varargin_1_idx_0;
  r1->size[1] = 1;
  ...
  loop_ub = y_mC->size[0];
  for (i9 = 0; i9 < loop_ub; i9++) {
    b_y_mC->data[i9] = y_mC->data[i9].re * (dVQ->data[i9] - r1->data[i9] *
      u_mC->data[i9].im);
  }

  rdivide(b_y_mC, r2, dVQ);
  i9 = r1->size[0] * r1->size[1];
  r1->size[0] = (int)varargin_1_idx_0;
  r1->size[1] = 1;
  emxEnsureCapacity_real_T(r1, i9);
  ...
  loop_ub = y_mC->size[0];
  for (i9 = 0; i9 < loop_ub; i9++) {
    b_y_mC->data[i9] = y_mC->data[i9].im * (r1->data[i9] * u_mC->data[i9].re -
      dVI->data[i9]);
  }

  emxFree_real_T(&r1);
  emxFree_real_T(&dVI);
  rdivide(b_y_mC, r2, dw2);
  i9 = dw->size[0];
