                   ...
                   const emxArray_creal_T *y_mC, double fs, double f0)
{
  ...
  emxInit_real_T(&r5, 2);
  if (u_mCal->size[0] < (int)idx_u_mCal_off->data[r4->data[0] - 1]) {
    i8 = r5->size[0] * r5->size[1];
    r5->size[1] = 0;
    emxEnsureCapacity_real_T(r5, i8);
  } else if (idx_u_mCal_off->data[r4->data[0] - 1] == idx_u_mCal_off->data
             [r4->data[0] - 1]) {
    ndbl = idx_u_mCal_off->data[r4->data[0] - 1];
    i8 = u_mCal->size[0];
    absb = r5->size[0] * r5->size[1];
    r5->size[0] = 1;
    r5->size[1] = (int)floor((double)i8 - ndbl) + 1;
    emxEnsureCapacity_real_T(r5, absb);
    absb = (int)floor((double)i8 - ndbl);
    for (i8 = 0; i8 <= absb; i8++) {
      r5->data[r5->size[0] * i8] = ndbl + (double)i8;
    }
  } else {
    ndbl = floor(((double)u_mCal->size[0] - idx_u_mCal_off->data[r4->data[0] - 1])
                 + 0.5);
    apnd = idx_u_mCal_off->data[r4->data[0] - 1] + ndbl;
    cdiff = apnd - (double)u_mCal->size[0];
    absa = (unsigned int)fabs(idx_u_mCal_off->data[r4->data[0] - 1]);
    absb = u_mCal->size[0];
    ...
    if (fabs(cdiff) < 4.4408920985006262E-16 * (double)u1) {
      ...
      apnd = idx_u_mCal_off->data[r4->data[0] - 1] + (ndbl - 1.0);
    } else {
      ndbl++;
    }
    ...
    emxEnsureCapacity_real_T(r5, i8);
    if (idx > 0) {
      r5->data[0] = idx_u_mCal_off->data[r4->data[0] - 1];
      if (idx > 1) {
        r5->data[idx - 1] = apnd;
        absb = (idx - 1) / 2;
        for (end = 1; end < absb; end++) {
          r5->data[end] = idx_u_mCal_off->data[r4->data[0] - 1] + (double)end;
          r5->data[(idx - end) - 1] = apnd - (double)end;
        }

        if (absb << 1 == idx - 1) {
          r5->data[absb] = (idx_u_mCal_off->data[r4->data[0] - 1] + apnd) / 2.0;
        } else {
          r5->data[absb] = idx_u_mCal_off->data[r4->data[0] - 1] + (double)absb;
          r5->data[absb + 1] = apnd - (double)absb;
        }
      }
    }
  }
  ...
  apnd = idx_u_mCal_off->data[r4->data[0] - 1];
  ndbl = (double)r5->size[1] - 1.0;
  i8 = b_u_mCal->size[0];
